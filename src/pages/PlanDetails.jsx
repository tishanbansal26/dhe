...
import { ArrowLeft, CheckCircle, Shield, ArrowRight, Check, Activity, Users, Stethoscope, BriefcaseMedical, Download, FileText, Share2, HelpCircle, ChevronDown, ChevronUp, AlertCircle, Info, Star, Sparkles } from 'lucide-react';
...
export default function PlanDetails() {
  ...
  const imageKeywords = plan.image_keywords || plan.category;
  ...
  return (
    <>
      ...
      <div className="pt-24 pb-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto min-h-screen">
        ...
        {/* Hero Section */}
        <div className="glass-panel rounded-3xl p-8 md:p-12 mb-12 border border-slate-700/50 bg-gradient-to-br from-slate-900 to-slate-800 relative overflow-hidden">
          
          {/* Background Image Overlay if Keywords Exist */}
          {imageKeywords && (
             <div 
               className="absolute top-0 right-0 w-1/2 h-full opacity-20 pointer-events-none mix-blend-overlay hidden md:block"
               style={{
                 backgroundImage: `url('https://source.unsplash.com/featured/?${encodeURIComponent(imageKeywords)}')`,
          ...
          <div className="flex flex-col md:flex-row gap-8 items-start md:items-center relative z-10">
            <div className="flex-1">
              ...
              {/* Eye-Catchy Highlights generated by AI */}
              {highlights.length > 0 && (
                <div className="mb-8 p-6 bg-slate-800/60 rounded-2xl border border-teal-500/20 backdrop-blur-sm">
                  <h3 className="text-teal-400 font-bold mb-4 flex items-center gap-2">
                    <Sparkles className="w-5 h-5" /> Why Choose This Policy?
                  </h3>
                  <ul className="space-y-3">
                    {highlights.map((highlight, idx) => (
                      <li key={idx} className="flex items-start gap-3">
                        <Star className="w-5 h-5 text-gold-400 shrink-0 mt-0.5 fill-gold-400/20" />
                        <span className="text-slate-200">{highlight.name}</span>
                      </li>
                    ))}
                  </ul>
                </div>
