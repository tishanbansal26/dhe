...
const categoryMeta = {
  health: {
    ...
    description: 'Compare comprehensive cashless health plans with zero room rent limits.'
  },
  life: {
    name: 'Term Life',
    ...
    description: 'Secure your family with pure term insurance plans and critical illness riders.'
  },
  investment: {
    name: 'Investment & Pension',
    icon: <TrendingUp className="w-8 h-8 text-emerald-400" />,
    color: 'from-emerald-500/20 to-teal-500/10',
    borderColor: 'border-emerald-500/30',
    incentiveBadge: 'Tax Saver 80C',
    description: 'Guaranteed lifetime annuity, guaranteed additions, and 100% return of purchase price.'
  },
  motor: {
    ...
    description: 'Zero depreciation bumper-to-bumper car & two-wheeler insurance.'
  }
};

const defaultCategories = ['health', 'life', 'investment', 'motor'].map(id => ({
  id,
  ...categoryMeta[id]
}));
...
export default function ProductGrid() {
  ...
  return (
    <section id="products" className="py-20 relative border-t border-white/5 bg-slate-900/20 scroll-mt-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
          ...
          <button 
            ...
            className="flex items-center gap-2 bg-slate-800 hover:bg-slate-700 text-white px-6 py-3 rounded-xl border border-slate-600 transition-colors font-semibold shadow-lg hover:border-teal-500/40"
          >
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[1, 2, 3, 4].map(i => (
              <div key={i} className="glass-panel rounded-3xl p-8 border border-slate-700/50 h-80 animate-pulse bg-slate-800/40"></div>
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {categories.map((cat) => (
              <div 
                ...
                className="group cursor-pointer relative glass-panel rounded-3xl p-8 border border-slate-700/50 hover:border-slate-500 transition-all duration-300 overflow-hidden flex flex-col h-full hover:shadow-2xl"
              >
                ...
                <div className="relative z-10 flex flex-col h-full">
                  ...
                  <p className="text-gray-400 text-sm mb-8 leading-relaxed">{cat.description}</p>
                  
                  <div className="mt-auto flex items-center justify-between pt-4 border-t border-slate-700/30">
                    <span className="text-teal-400 font-semibold group-hover:text-teal-300 text-sm">View Plans</span>
